...
class DeepSeekProvider(LLMProvider):
    """DeepSeek's API is OpenAI-compatible — same SDK, different base_url and
    key. Much cheaper than OpenAI/Anthropic, a good low-cost default."""
    ...
    def _chat(self, messages: list[ChatMessage], model: str, temperature: float, max_tokens: int, json_mode: bool):
        try:
            ...
            raise LLMProviderError(f"DeepSeek authentication failed: {error}") from error
        except APITimeoutError as error:
            raise LLMProviderError(f"DeepSeek request timed out: {error}") from error
